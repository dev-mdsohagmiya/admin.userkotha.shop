...
const PermissionDebug = () => {
  ...
  const materialsPerms = useModulePermissions("Materials");

  const modulePermByName = {
    Products: productsPerms,
    Sales: salesPerms,
    Materials: materialsPerms,
  } as const;
          ...
          <Card title="Permission Tests" className="mb-4">
            <div className="space-y-3">
              {testsPermissions.map((test, idx) => {
                const row =
                  modulePermByName[
                    test.module as keyof typeof modulePermByName
                  ];
                const result = row
                  ? moduleHasAction(row.allActions, test.action)
                  : false;
                return (
                  <div
                    key={idx}
