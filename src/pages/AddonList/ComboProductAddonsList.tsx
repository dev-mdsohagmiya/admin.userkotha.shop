...
const ComboProductAddonsList = () => {
  const { id } = useParams();
  const [page, setPage] = useState(1);
  const [limit, setLimit] = useState(10);

  // Fetch combo product details for header
  const { data: productData } = useGetComboProductByIdQuery(id as string, {
    skip: !id,
  });

  // Fetch addons for this combo product
  // Assuming the API supports filtering by comboProductId
  const {
    data: addonData,
    isLoading,
    isFetching,
  ...
  return (
    <div>
      ...
      <div className="">
        <PageHeader
          title={`Addons for ${productData?.data?.name || "Combo Product"}`}
          subtitle="Manage combo product addons"
          breadcrumbs={[
      ...
      {openAddonModal && (
        <AddonModal
          ...
          defaultSourceComboProductId={id}
        />
      )}
    </div>
  );
...
export default ComboProductAddonsList;
