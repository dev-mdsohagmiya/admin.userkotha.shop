...
interface FolderTreeProps {
  ...
  isOpen?: boolean;
  onToggle?: () => void;
  /** When false, hide all folder-create UI (sidebar uses separate props from rename/delete). */
...
const FolderTree: React.FC<FolderTreeProps> = ({
  ...
  const handleCreateFolder = (parentPath: string) => {
    ...
    // Only proceed if we are still in creating state
  ...
  const handleRenameFolderNode = (path: string) => {
    ...
    // Only proceed if we are still in editing state
    if (editingFolder === path) {
        ...
        >
          ...
          {editingFolder === folder.path ? (
            <Input
              ...
              onPressEnter={() => handleRenameFolderNode(folder.path)}
              onBlur={() => handleRenameFolderNode(folder.path)}
              autoFocus
            ...
            <div className="flex-1 flex items-center justify-between group min-w-0">
              ...
              >
                ...
                {folder.path !== "Root" &&
                  (getFolderItems(folder.path)?.length ?? 0) > 0 && (
                    <Dropdown
  ...
  return (
    <div className="border border-gray-200 rounded-lg bg-white h-full overflow-hidden flex flex-col">
      {/* Header */}
