...
import { useCreatePackagingMaterialMutation } from "../../../../redux/features/packaging-material/packagingMaterialApi";
...
interface CreatePackagingMaterialModalProps {
  open: boolean;
  setOpen: React.Dispatch<React.SetStateAction<boolean>>;
}

const CreatePackagingMaterialModal: React.FC<
  CreatePackagingMaterialModalProps
> = ({ open, setOpen }) => {
  const [form] = Form.useForm<IPackagingMaterial>();

  // API hooks
  const [createPackagingMaterial, { isLoading }] =
  ...
  const onFinish = async (values: IPackagingMaterial) => {
    try {
      ...
      const res = await createPackagingMaterial(payload).unwrap();

      if (res?.success) {
        toast.success(res?.message || "Packaging created successfully!");
        navigate("/packaging-materials"); // navigate to units page
        setOpen(false);
        form.resetFields();
      }
    } catch (err: any) {
      ...
      title={
        <div className="mb-2">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Fill out the details to Create Packaging Material.
          </p>
        </div>
      }
    >
...
export default CreatePackagingMaterialModal;
